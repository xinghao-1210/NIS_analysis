function [stats,x,y,alldata] = hist2d_NIS_objdata(pxy,addtext,multi_thresh,sel_cond,datapts,xlsfile,grps)
%function [stats,x,y,alldata] = hist2d_NIS_objdata(pxy,addtext,multi_thresh,sel_cond,datapts,xlsfile,grps)
%   pxy=[px,py] doing gating
%   addtext=[add percentages to graph, add total counts to graph (1=true,0=false)
...
if (iscell(datapts) && length(datapts)<3) || (~iscell(datapts))
    if iscell(datapts)
    ...
    datapt(1)=select_ctrl_GUI('Select plot',head,'Select channel to plot along the X-axis:');
    datapt(2)=select_ctrl_GUI('Select plot',head,'Select channel to plot along the Y-axis:');
end
    ...
    for m=1:length(tempg)
        ...
        if ~isempty(temp_chan_idx)
            ...
            if temp_chan_idx<=length(alldata(n).data(m).chan_dat)
                if multi_thresh<1
                    eval(['data{n,1}=[data{n,1};alldata(n).data(m).chan_dat(temp_chan_idx).',head{datapt(1)},'];'])
                    eval(['data{n,2}=[data{n,2};alldata(n).data(m).chan_dat(temp_chan_idx).',head{datapt(2)},'];'])
                    eval(['data_grp_by_rep{n,m,1}=alldata(n).data(m).chan_dat(temp_chan_idx).',head{datapt(1)},';'])
                    eval(['data_grp_by_rep{n,m,2}=alldata(n).data(m).chan_dat(temp_chan_idx).',head{datapt(2)},';'])
                else
                    num_threshing=0;
                    ...
                    eval(['data{n,1}=[data{n,1};alldata(n).data(m).chan_dat(temp_chan_idx).',head{datapt(1)},'(curridx_thresh)];'])
                    eval(['data{n,2}=[data{n,2};alldata(n).data(m).chan_dat(temp_chan_idx).',head{datapt(2)},'(curridx_thresh)];'])
                    eval(['data_grp_by_rep{n,m,1}=alldata(n).data(m).chan_dat(temp_chan_idx).',head{datapt(1)},'(curridx_thresh);'])
                    eval(['data_grp_by_rep{n,m,2}=alldata(n).data(m).chan_dat(temp_chan_idx).',head{datapt(2)},'(curridx_thresh);'])
                end
            end
        end
    end
...
for k=1:length(sel_cond)
    f(k)=figure;hold on
    n=sel_cond(k);
    dscatter_sltfix(data{n,1},data{n,2},'MSIZE',markersize,'MARKER',markershape);
...
for k=1:length(sel_cond)
    ...
    if add_perc2graph || add_tot2graph
    ...
    if select_stats_threshold
        plot([px,px],[0 5000],'k','LineWidth',1)
        plot([0 5000],[py,py],'k','LineWidth',1)
    end
    set(f(k),'color',[1,1,1])
