...
extern mclComponentData __MCC_hist2d_gui_v2_2_component_data;
 ...
 */
#ifndef LIB_hist2d_gui_v2_2_C_API 
#define LIB_hist2d_gui_v2_2_C_API /* No special import/export declaration */
#endif

LIB_hist2d_gui_v2_2_C_API 
bool MW_CALL_CONV hist2d_gui_v2_2InitializeWithHandlers(
...
)
{
  ...
  if (!mclInitializeComponentInstanceWithEmbeddedCTF(&_mcr_inst,
                                                     &__MCC_hist2d_gui_v2_2_component_data,
                                                     true, NoObjectType,
                                                     ExeTarget, error_handler,
                                                     print_handler, 136431, NULL))
    return false;
  return true;
}

LIB_hist2d_gui_v2_2_C_API 
bool MW_CALL_CONV hist2d_gui_v2_2Initialize(void)
{
  return hist2d_gui_v2_2InitializeWithHandlers(mclDefaultErrorHandler,
                                               mclDefaultPrintHandler);
}

LIB_hist2d_gui_v2_2_C_API 
void MW_CALL_CONV hist2d_gui_v2_2Terminate(void)
{
  if (_mcr_inst != NULL)
...
int run_main(int argc, const char **argv)
{
  ...
  __MCC_hist2d_gui_v2_2_component_data.path_to_component = path_to_component; 
  if (!hist2d_gui_v2_2Initialize()) {
    return -1;
  }
  argc = mclSetCmdLineUserData(mclGetID(_mcr_inst), argc, argv);
  _retval = mclMain(_mcr_inst, argc, argv, "hist2d_gui", 1);
  if (_retval == 0 /* no error */) mclWaitForFiguresToDie(NULL);
  hist2d_gui_v2_2Terminate();
...
int main(int argc, const char **argv)
{
  if (!mclInitializeApplication(
    __MCC_hist2d_gui_v2_2_component_data.runtime_options,
    __MCC_hist2d_gui_v2_2_component_data.runtime_option_count))
    return 0;
